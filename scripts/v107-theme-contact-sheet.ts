...
try {
  ...
  for (let i = 0; i < themes.length; i++) {
    ...
    const html = `<!doctype html><html lang="pl"><head><meta charset="utf-8"><style>${baseCss}\n${runtime.css}\nhtml,body{background:#f4f0e8}body{margin:0!important;padding:0!important}.preview-card{width:412px;min-height:680px;margin:20px auto;background:#fff;box-shadow:0 10px 28px rgba(0,0,0,.14);overflow:hidden}.preview-card main.book{box-sizing:border-box;max-width:none;width:100%;min-height:680px;padding:36px 34px 50px}.preview-card section.level1{margin-top:0!important;border-top:0!important;padding-top:0!important;break-before:auto!important;page-break-before:auto!important}.preview-card p{font-size:16px}.preview-card h1{margin-top:0}</style></head><body><div class="preview-card"><main class="book"><section class="chapter level1"><h1>Miasto po deszczu</h1>${firstParagraph(theme.dropcap)}<p>${paragraph2}</p><p class="scene-break">${theme.sceneOrnament}</p><p>— Czy naprawdę możemy tam wrócić? — zapytała. — Możemy, ale nie powinniśmy udawać, że niczego się nie boimy.</p></section></main></div></body></html>`;
