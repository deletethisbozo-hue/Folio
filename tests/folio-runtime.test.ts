...
const expectedThemes = ["blackletter", "stanza", "witchlight", "solstice", "literary", "nocturne", "obsidian", "grimoire", "ivory", "heritage", "decorative", "cathedral", "aubade", "event-horizon", "signal", "nova", "redline", "intercept", "black-ice", "noir", "dread", "seance", "velvet", "amour", "hearth", "regency", "voyager", "afterglow", "academia", "memoir"];
check("Folio registers exactly the 30 production themes", themes.body.length === expectedThemes.length && expectedThemes.every((name) => themes.body.some((theme: any) => theme.name === name)), themes.body.map((theme: any) => theme.name).join(", "));
