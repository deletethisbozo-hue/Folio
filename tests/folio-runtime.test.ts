...
const expectedThemes = ["amour", "seance", "memoir", "gothic", "scriptorium", "arcana", "blackletter", "stanza", "witchlight", "solstice", "literary", "nocturne", "obsidian", "grimoire", "ivory", "heritage", "decorative", "cathedral", "aubade"];
check("Folio 2.1.3 registers exactly the 19 curated themes", themes.body.length === expectedThemes.length && expectedThemes.every((name) => themes.body.some((theme: any) => theme.name === name)), themes.body.map((theme: any) => theme.name).join(", "));
