...
try {
  ...
  await page.waitForSelector(".reader-device.device-iphone");
  await page.waitForFunction(() => {
  ...
  await page.waitForSelector(".add-chapter-box input");
  ...
  await page.waitForFunction(() => [...document.querySelectorAll(".contents-row")].some((row) => row.textContent?.includes("UI Added Chapter")));
  ...
  await page.waitForSelector(".empty-project-editor");
  ...
  await page.waitForFunction(() => document.querySelector("iframe")?.contentDocument?.body?.innerText.includes("The book can now be written."));
  ...
  await page.waitForFunction(() => document.querySelector(".book-title")?.textContent === "Born Tied");
