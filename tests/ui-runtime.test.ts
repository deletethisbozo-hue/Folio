...
try {
  ...
  await stage("deleted-chapter marker preview", async () => {
    try {
      await page.waitForFunction(() => document.querySelector("iframe")?.contentDocument?.body?.textContent?.replace(/\u00ad/g, "").includes("DELETED CHAPTER PREVIEW MARKER"));
    } catch (error) {
      const snapshot = await page.evaluate(() => {
  ...
  await stage("deleted chapter removed from preview", () => page.waitForFunction(() => !document.querySelector("iframe")?.contentDocument?.body?.textContent?.replace(/\u00ad/g, "").includes("DELETED CHAPTER PREVIEW MARKER")));
