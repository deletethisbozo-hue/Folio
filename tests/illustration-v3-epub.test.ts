...
console.log("\nFolio illustration V3 contour EPUB");
...
try {
  ...
  chapter.markdown = [
    ...
    "",
    "![Anchored EPUB illustration](assets/anchored-epub.png){.folio-illustration .folio-wrap-left .folio-shape-contour width=38% data-folio-gap=70}",
    "",
  ...
  check("V3 contour illustration survives into EPUB XHTML", Boolean(host), host?.name ?? "missing XHTML host");
  ...
  check("EPUB preserves contour semantic class", /folio-shape-contour/.test(hostText));
  check("EPUB resolves contour against packaged image src", /shape-outside\s*:\s*url\('[^']*(?:media|file)\d*\.png'\)/i.test(hostText), hostText.match(/shape-outside[^;"']*[^"]*/i)?.[0] ?? "shape-outside missing");
  check("EPUB preserves contour gap as shape-margin", /shape-margin\s*:\s*0\.7em/i.test(hostText), hostText.match(/shape-margin[^;"]*/i)?.[0] ?? "shape-margin missing");
  ...
  failed++;
  console.error("✗ V3 contour EPUB scenario completed");
  console.error(error);
} finally {
